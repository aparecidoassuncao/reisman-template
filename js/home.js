...
//   },1000)
  ...
  function changeImgCard() {
    document.querySelectorAll('.receiveImgs > .outlineColorImage').forEach((image) => {
      image.addEventListener('click', function () {
        // Mudar a classe ativa
        this.parentElement.querySelectorAll('.active').forEach(sibling => sibling.classList.remove('active'));
        ...
        if (productBlock) {
          productBlock.querySelector('.img-card .principal').src = selectedColor;
          let selectedSecond = this.getAttribute('data-imagens-second');
          productBlock.querySelector('.img-card .second-img').src = selectedSecond;

          // Mudar o link
          let selectedColorUrl = this.getAttribute('data-url');
          productBlock.querySelector('a').setAttribute('href', selectedColorUrl);

          // Mudar o preço
          let selectedPriceItem = this.getAttribute('data-price');
          productBlock.querySelector('.preco-produto-spot').textContent = selectedPriceItem;

          let selectedNameItem = this.getAttribute('data-nameColor');
          productBlock.querySelector('.name-color-product span').textContent = selectedNameItem;
...
function newPriceCard() {
  document.querySelectorAll('.product-block .description .preco-produto-spot').forEach(function (element) {
  ...
  document.addEventListener("click", function(event) {
    ...
    setTimeout(function() {
    document.querySelector(".header-search .header-search-wrapper .form-control").focus();
}, 500);

  });

  document.addEventListener("click", function(event) {
    const searchItemClose = event.target.closest(".header-search.actived button.close-seach");
  ...
  document.querySelector('.logo-menu button.close-icon.ta-header-menu-close').addEventListener('click', function() {
    document.querySelector('.menu-mobile.nav-mobile').classList.remove('active');
  });
