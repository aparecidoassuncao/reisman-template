  ...
  if($('.checkout-cart-preview .canopus-cart-items ul li .canopus-cart-item-image').length > 0){
    $('.checkout-cart-preview #div-observacao').removeClass('hide');
     $('.frete-wrapper.modal-subtotal.modal-preview-bottom-item').removeClass('hide');
       ...
       price = parseFloat($('.modal-subtotal.modal-preview-bottom-item span.cart-subtotal').text().replace("R$", "").replace(".", "").replace(",", ".").trim())
...
function adicionarClassesIncrementais() {
    ...
    elementos.forEach(function(elemento) {
        ...
        var baseClass = classList.find(cls => cls.startsWith('arrayGravacoes-'));
        if (baseClass) {
            ...
            if (count > 0 && count <= sufixos.length) {
                elemento.classList.add('arrayGravacoes-' + baseId + '-' + sufixos[count - 1]);
            }
...
adicionarClassesIncrementais();
   ...
   $(document).on("click", "#btn-clear-cart", function () {
                ...
                console.log('Local storage items starting with "arrayGravacoes" have been cleared.');
            });
