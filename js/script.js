...
$('.slick-prev').each(function(){
  ...
  img.attr('src', 'https://assets.betalabs.net/production/reisman/images/stores/1/arrow-left.png');
	$(this).append(img);
});
...
$('.slick-next').each(function(){
  $(this).html('');
  var img = $(document.createElement('img'));
  img.attr('src', 'https://assets.betalabs.net/production/reisman/images/stores/1/arrow-right.png');
	$(this).append(img);
});
    ...
    function rodarScriptAddCarrinho(nvezescarregou) {
        ...
        if(nvezescarregou > 0) {
            ...
            $('.coluna-spot-category.carregou-'+nvezescarregou).first().attr('id', 'carregou');
            $('html,body').unbind().animate({scrollTop: $('#carregou').offset().top-alturatopo});
        }
    }
            ...
            setTimeout(function(){
                var interval = setInterval(function(){
                    if($('span.loader').length == 0) {
                        ...
                        $(this).parent().next().find('.nome-produto-spot').text(selectedNameItem)
