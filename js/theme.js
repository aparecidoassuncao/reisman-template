function submitUserForm() {
    ...
    if(response.length == 0) {
        document.getElementById('g-recaptcha-error').innerHTML = '<span style="color:red;">Preencha o Recaptcha.</span>';
        return false;
    }
    return true;
}
...
(function() {
  ...
  fnAttachFieldKeyUp();
  document.querySelector('.searchForm form.busca-Search #cat').onchange = function() {
      fnAttachFieldKeyUp();
  }
})()
 ...
 CanopusEventCenter.addListener('ServerDataLoader.Loaded', function (data) {
  if (typeof data.client !== undefined) {
    ...
    if (typeof data.cart !== undefined) {
      ...
      $('.carrinho-subtotal').html((parseInt(data.cart.subtotal * 100) / 100).toLocaleString('pt-BR', {
