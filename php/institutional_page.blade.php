...
@section('metadescription', $pagina->model->description )
    ...
    <script src='https://snapppt.com/widgets/widget_loader/ad989e76-889f-4067-a793-727bb1699d1d/grid.js' defer class='snapppt-widget'></script>
    @else
    <div id="pagina-institucional-content" style="margin-top: 0px">
        ...
        @endif
            <div id="content-institucional">
                ...
                @if(starts_with(request()->path(), 'contato') || starts_with(request()->path(), 'garantia') || starts_with(request()->path(), 'suporte') || starts_with(request()->path(), 'trabalhe-conosco'))
                <form id="form-contato" action="#" method="POST" onsubmit="return submitUserForm();">
                  <input type="text" class="form-control" name="nome" id="nome" placeholder="Seu nome" required>
                  <input type="email" class="form-control" name="email" id="email" placeholder="Seu e-mail" required>
                  <input type="phone" class="form-control" name="telephone" id="telephone" placeholder="Seu telefone" required>
                  @if(starts_with(request()->path(), 'contato'))
                  <input type="text" class="form-control hide" name="assunto" id="assunto" value="Fale Conosco">
                  ...
                  <input type="text" class="form-control hide" name="assunto" id="assunto" value="Trabalhe Conosco">
                  @endif
                  <textarea id="form-textarea" class="form-control" name="mensagem" id="mensagem" rows="8" placeholder="Sua mensagem" required></textarea>
          ...
          <div class="g-recaptcha" data-sitekey="6Leayb8qAAAAAFRsWsfcgqUCpijwf64723o4OA-N"></div>
                ...
                @elseif(starts_with(request()->path(), 'depoimentos'))
                    @if($pagdepoimentos->records->count() > 0)
                      <div id="pagina-depoimento" class="container text-center">
                        <div class="row">
                          @foreach($pagdepoimentos->records->sortBy(function($depoimento){if($depoimento->ordem_depoimento !== null){
                            return $depoimento->ordem_depoimento->values[0]->value;}}) as $depoimento)
                ...
                <div class="div-form">
                    <form id="form-agendamento" enctype="multipart/form-data" name="form-agendamento">
                      <input type="text" class="form-control" name="nome_agendamento" placeholder="Seu nome" id="nome" required>
                      <input type="email" class="form-control" name="e_mail_agendamento" placeholder="Seu e-mail" id="email" required>
                      <input type="phone" class="form-control" name="telefone_agendamento" placeholder="Seu telefone" id="phone" required>
                      <input type="text" class="form-control" name="joia_de_interesse_agendamento" placeholder="Joia de interesse" id="joia" required>
                      <input type="text" class="form-control" name="data_agendamento" placeholder="Data" id="data" required>
                          ...
                          <select class="form-control" name="horario_agendamento" id="horario" required>
                            ...
                            @foreach($horariosdeagendamento->records as $horario)
                              @if($horario->horario_de_agendamento != null)
                                <option value="{{ $horario->horario_de_agendamento->values->first->value->value }}">{{ $horario->horario_de_agendamento->values->first->value->value }}</option>
                              @endif
                            @endforeach
                          </select>
                        @endif
                      <textarea class="form-control" name="observacao_agendamento" id="observacao" rows="8" placeholder="Observação (opcional)"></textarea>
                      <button type="submit" class="btn btn-enviar-mensagem">Enviar solicitação de agendamento</button>
                    </form>
          ...
          <div class="g-recaptcha" data-sitekey="6Leayb8qAAAAAFRsWsfcgqUCpijwf64723o4OA-N"></div>
                ...
                @endif
                @if(!starts_with(request()->path(), 'depoimentos') && !starts_with(request()->path(), 'fabricacao') && !starts_with(request()->path(), 'trabalhe-conosco'))
                    @if($depoimentos->records->count() > 0)
                      <div id="pagina-depoimento" class="container text-center">
                        ...
                        <div class="row">
                          @foreach($depoimentos->records->sortBy(function($depoimento){if($depoimento->ordem_depoimento !== null){
                            return $depoimento->ordem_depoimento->values[0]->value;}}) as $depoimento)
                            @include(@engine_view('div-depoimentos'))
                          @endforeach
                        </div>
    ...
    CanopusEventCenter.addListener('ServerDataLoader.Loaded', function (data) {
        ...
        if (typeof data.client !== undefined) {
          ...
          if (estaLogado) {
              ...
              iziToast.info({title: 'Aviso', message: 'Para prosseguir é necessario estar logado!',buttons: [
                 ['<button><b>Criar conta</b></button>', function (instance, toast) {
      ...
      $('#form-agendamento').on('submit', function(e) {
        ...
        if(response.length == 0) {
            document.getElementById('g-recaptcha-error').innerHTML = '<span style="color:red;">Este campo é obrigatório.</span>';
            return false;
        } else {
            ...
            $.ajax({
              ...
              'success': function(info) {
                  iziToast.success({title: 'Cadastro enviado!', message: 'Você receberá um retorno em breve, obrigado!'})
              },
              'error': function(erro){
                  iziToast.warning({title: 'Aviso', message: 'Erro ao enviar solicitação. Tente novamente mais tarde'});
              }
            });
        }
      });
