...
@if ($item->extraFields && $item->record && $item->record->extraFields && $item->record->extraFields->has('thumb_produto'))
<div class="outlineColorImage {{ url($item->record->urls->first()->url) === url($item->urls->first()->url) ? 'active' : '' }}"
    ...
    data-imagens-second="
@if($item->record->extraFields->has('fotos_novas') && $item->record->extraFields->get('fotos_novas')->values->first()->extraFieldOption->label == 'Sim')
    @if($item->record->extraFields->has('imagem_produto_05'))
