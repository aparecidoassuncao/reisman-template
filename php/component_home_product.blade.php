...
<div class="section-products first-vitrine">
    <div class=" container">
        <div class="header-main">
            ...
            <div class="arrows-vitrine">
                <div class="prev-card">
                <svg xmlns="https://www.w3.org/2000/svg" version="1.1" xmlns:xlink="https://www.w3.org/1999/xlink" width="30" height="30" x="0" y="0" viewBox="0 0 128 128" style="enable-background:new 0 0 512 512" xml:space="preserve" class=""><g transform="matrix(-1.8369701987210297e-16,1,-1,-1.8369701987210297e-16,127.99968910217285,0.00006294250494676135)"><path d="M64 88a3.988 3.988 0 0 1-2.828-1.172l-40-40c-1.563-1.563-1.563-4.094 0-5.656s4.094-1.563 5.656 0L64 78.344l37.172-37.172c1.563-1.563 4.094-1.563 5.656 0s1.563 4.094 0 5.656l-40 40A3.988 3.988 0 0 1 64 88z" fill="#000000" opacity="1" data-original="#000000" class=""></path></g></svg>
                </div>
                <div class="next-card">
                 <svg xmlns="https://www.w3.org/2000/svg" version="1.1" xmlns:xlink="https://www.w3.org/1999/xlink" width="30" height="30" x="0" y="0" viewBox="0 0 128 128" style="enable-background:new 0 0 512 512" xml:space="preserve" class=""><g transform="matrix(6.123233995736766e-17,-1,1,6.123233995736766e-17,-0.00006294250488991793,127.99968910217288)"><path d="M64 88a3.988 3.988 0 0 1-2.828-1.172l-40-40c-1.563-1.563-1.563-4.094 0-5.656s4.094-1.563 5.656 0L64 78.344l37.172-37.172c1.563-1.563 4.094-1.563 5.656 0s1.563 4.094 0 5.656l-40 40A3.988 3.988 0 0 1 64 88z" fill="#000000" opacity="1" data-original="#000000" class=""></path></g></svg>
                </div>
            </div>
        </div>
                        ...
                        <div class="product-block custom-css">
                            <a @if($itemFavorito->urls->first()) href="{{url($itemFavorito->urls->first()->url)}}" @endif>
                                <figure class="img-card image -vertical">
                                    <img class="principal"
                                        @if($itemFavorito->extraFields->has('thumb_produto'))
                                    src="https://assets.betalabs.net/production/reisman/{{ $itemFavorito->extraFields->get('thumb_produto')->values->first->source->file_path }}"
                                    @else
                                    src="{{ thumb($itemFavorito->main_image->source) }}"
                                    @endif
                                    alt="{{ ($cat = collect($itemFavorito->categories)
        ->sortBy('id')
        ->filter(fn($cat) => 
    ...
    ? (
        ...
        : (
            $cat->id == 2 
                ? 'Exclusividade do ' . $itemFavorito->identification->first()->name . ' com visual único e detalhes sofisticados'
            : (
                $cat->id == 3 
                    ? 'Destaque para ' . $itemFavorito->identification->first()->name . ' com design sofisticado e acabamento elegante'
                : (
                    $cat->id == 6 
                        ? 'Sofisticação do ' . $itemFavorito->identification->first()->name . ' com toque elegante e acabamento diferenciado'
                    : (
                        $cat->id == 45 
                            ? 'Texto para categoria 45 mais ' . $itemFavorito->identification->first()->name . '.'
                        : (
                            $cat->id == 55 
                                ? 'Beleza do ' . $itemFavorito->identification->first()->name . ' com brilho marcante e design contemporâneo'
    ...
    : 'Imagem do Produto ' . $itemFavorito->identification->first()->name . ' '}}" style="opacity: 1 !important;">
...
@if($itemFavorito->extraFields->has('agrupamentos_de_produtos_1') && $itemFavorito->extraFields->get('agrupamentos_de_produtos_1') !== null)
<div class="outlineColorImage active main-image"
    ...
    data-nameColor="@if($itemFavorito->extraFields->get('selecionar_cor_do_ouro') && $itemFavorito->extraFields->get('selecionar_cor_do_ouro')->values->isNotEmpty())
...
@endif"
    ...
    <div class="nameItem">
        <span>
            @if($itemFavorito->extraFields->get('selecionar_cor_do_ouro') && $itemFavorito->extraFields->get('selecionar_cor_do_ouro')->values->isNotEmpty())
    {{ $itemFavorito->extraFields->get('selecionar_cor_do_ouro')->values->first()->extraFieldOption->label }}
@endif
        </span>
    </div>
...
@if($itemFavorito->extraFields->has('agrupamentos_de_produtos') && $itemFavorito->extraFields->get('agrupamentos_de_produtos') !== null)
...
@if ($itemFavorito->extraFields && $value->record && $value->record->extraFields && $value->record->extraFields->has('thumb_produto'))
...
@endif
@if($itemFavorito->extraFields->has('agrupamentos_de_produtos_2') && $itemFavorito->extraFields->get('agrupamentos_de_produtos_2') !== null)
...
@endif
  ...
  @if($itemFavorito->extraFields->get('selecionar_flag') && $itemFavorito->extraFields->get('selecionar_flag')->values->isNotEmpty())
